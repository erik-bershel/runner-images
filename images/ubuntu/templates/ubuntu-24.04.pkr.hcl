...
variable "plan_info" {
  ...
  default = {
    plan_name      = "github_arm_linux_runner_plan_2404"
    plan_product   = "github_arm_linux_runner_2404"
    plan_publisher = "arm"
  }
}
...
build {
  ...
  provisioner "shell" {
    execute_command = "sudo sh -c '{{ .Vars }} {{ .Path }}'"
    inline          = ["touch ${var.image_folder}/software-report.md", "touch ${var.image_folder}/software-report.json"]
  }
